...
export class EventBookingFormContainer extends React.Component<
  EventBookingFormContainerProps,
...
> {
  ...
  render() {
    ...
    return (
      <EventBookingForm
        ...
        onSubmit={(amount) => this.handleSubmit(amount)}
      />
    );
  }
  ...
  private handleSubmit(seats: number) {
    ...
    fetch(url, { method: "POST", body: JSON.stringify(payload) }).then(
      (response) => {
        console.log("foo");
        this.setState({
          event: this.state.event,
